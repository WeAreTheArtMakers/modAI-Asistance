...
export function DesktopHandoff(t0) {
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("modAI Desktop is not installed.");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`modAI Desktop needs to be updated (found v${installStatus.version}, need v1.1.2396+).`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
        ...
        if (!result.success) {
          setError(result.error ?? "Failed to open modAI Desktop");
          setState("error");
          return;
        }
        setState("success");
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      checking: "Checking for modAI Desktop\u2026",
      flushing: "Saving session\u2026",
      opening: "Opening modAI Desktop\u2026",
      success: "Opening in modAI Desktop\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
...
async function _temp2(onDone_0) {
  onDone_0("Session transferred to modAI Desktop", {
    display: "system"
  });
